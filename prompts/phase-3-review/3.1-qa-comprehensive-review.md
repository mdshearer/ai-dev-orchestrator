...
✅ **Use when:** You've completed a feature/task and want broad feedback before merging
❌ **Don't use when:** You want deep analysis of one specific aspect (use specialized prompts 3.2-3.5)
...
```markdown
You are a Senior QA Engineer. Review the following **[PRIMARY_LANGUAGE]** code:
...
- Replace `[PRIMARY_LANGUAGE]` → e.g., "Python", "TypeScript"
- Replace `[PASTE_YOUR_CODE_HERE]` → Copy your code

### Step 3: Invoke AI
Paste the filled prompt with:
- Your code
- CONSTITUTION.md
- Any related context files

### Step 4: Address Issues by Priority

**Priority order:**
1. 🔴 **CRITICAL** - Fix immediately (security, crashes, data loss)
2. 🟠 **HIGH** - Fix before merging (bugs, missing error handling)
3. 🟡 **MEDIUM** - Fix this iteration (edge cases, quality issues)
4. 🟢 **LOW** - Fix when convenient (nitpicks, small optimizations)
...
|----------|-----------|---------|
| `[PRIMARY_LANGUAGE]` | AI (from CONSTITUTION.md) or Human | "Python", "TypeScript" |
| `[PASTE_YOUR_CODE_HERE]` | Human at use-time | Your code to review |
